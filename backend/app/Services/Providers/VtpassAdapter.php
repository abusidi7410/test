...
class VtpassAdapter implements ProviderServiceInterface
{
    private VtuProvider $provider;
    ...
    public function testConnection(): array
    {
        try {
            ...
            ])->timeout(15)->get($this->baseUrl . '/api/service-variations', [
                'serviceID' => 'mtn',
            ]);
            ...
            if ($response->successful() && isset($body['content']['variations'])) {
                return [
                    'success' => true,
                    'message' => 'Connection successful',
                    'response_time_ms' => $elapsed,
                    'status_code' => $response->status(),
                    'variations_count' => count($body['content']['variations'] ?? []),
                ];
            }

            return [
                'success' => false,
                'message' => $body['response_description'] ?? $body['message'] ?? 'Unexpected response',
                'response_time_ms' => $elapsed,
                'status_code' => $response->status(),
            ];
        } catch (\Exception $e) {
